...
export class SignUpComponent implements OnInit, OnDestroy {
  ...
  public readonly email = new FormControl<string>('', [Validators.required, Validators.email]);
  ...
  public readonly password = new FormControl<string>('', [
    ...
    this.validationService.checkForForbiddenValues(() =>
      [this.firstName.value, this.lastName.value, this.email.value].filter(value => value !== undefined),
    )
  ]);
  ...
  private readonly backendCommunicationService = inject(BackendCommunicationService);
  ...
  private sendDataToBackend(): Subscription {
    this.form.disable();
    return this.backendCommunicationService.signUp(this.form.getRawValue()).pipe(
      catchError(() => {
        this.errorMessage.set(this.translocoService.translate('friendlyErrorMessage'));
        return EMPTY;
      }),
